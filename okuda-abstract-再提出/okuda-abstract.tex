...
\begin{nttdef-a}\label{nttdef:setting}
  ...
  \begin{itemize}
  \item $G$を非コンパクト実線型簡約Lie 群，$K$は$G$のCartan対合$\Theta$に対して$\Theta K = K$なる$G$の極大コンパクト部分群とする．$H$は$G$の非コンパクトかつ連結成分有限個の閉部分群で$H = \Theta H$を満たすものとする．
...
まず$G$が実階数1の実線型半単純Lie群，$\dim \ha\cap \pe = 1$の場合は$G = \SU(1,2) $，$H= \SO(1,1)$あるいは$G = \SU(1,1) $，$H= \SO(1,1)$の場合に帰着することで\Cref{prob:1121}の肯定的な例であることを証明した ($G$の線型性の仮定は外すことができる)．
