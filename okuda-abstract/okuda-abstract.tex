...
\end{prop-a}
\vspace{-0.5em}
\Cref{prop:reduction}は\cite[p.~409]{hel01}の{$\SU(2,1) $-reduction}と\cite{yos38}の定理を併せて示される．
...
\begin{thm-a}(\cite[pp.~665--6]{ber88})
  $G/H$のランクが$d$であるとするとき，$G$の既約ユニタリ表現$V$が$G$の正則表現$L^2(G/H)$の既約分解に出現する必要条件は，非自明な$G$絡作用素$\alpha_V\colon (C_c(G/H))^{\infty}\to V $が存在し，かつ任意の$v\in V^{\infty} $，$d' > d$に対して$\dint_{G/H}\lbig|\beta_V(v)(x)(1+r(x))^{-d/2} \rbig|^2\; dx < \infty $なることである．% ただし$\beta_V $は次の命題により$\alpha_V $と対応する$G$絡作用素$\beta_V\colon V^{\infty}\to C(G/H)^{\infty}  $とする．
\end{thm-a}
