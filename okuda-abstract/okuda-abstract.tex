...
まずいくつか用語を準備する．$G$を実簡約Lie群，$H$を$G$の閉部分群とし，$G/H$には左Haar測度$\mu_{G/H} $が存在すると仮定する．局所有界関数$r\colon G\to \real_{\geq 0} $がproperなradial functionであるとは，$r$が次を満たすことである．
  ...
  \begin{prop-a}(\cite[p.~678]{ber88})
    $G/H$の左Haar測度$\mu_{G/H} $を1つ固定する．このとき，$\Hom_{G}((C_c(G/H))^{\infty}, V )\to \Hom_{G}(V^{\infty}, C(G/H)^{\infty}) $，$\alpha_V\mapsto \beta_V\st \lyama v,\alpha_V(\phi)\ryama_{V} = \lyama \beta_{V}(v)\mu_X , \phi\ryama_{} $，$v\in V$，$\phi \in  (C_c(G/H))^{\infty} $
  \end{prop-a}
