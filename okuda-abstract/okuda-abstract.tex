...
$G$を実簡約Lie群，$H$を$G$の閉部分群とし，ある可換部分群$B\subset G$が存在して$G = KBH $というCartan分解を持つとする．任意の$X\in \Lie B$に対して$Y(\real X) $が有界であれば，$G/H$がランク$d \defeq \dim B $となるための条件のうちの1つである，「ある定数$C > 0$が存在して，任意の$X\in \be$に対し，$d_{G/K}(\exp(-X)\cdot o_K, o_K )  - \inf\{d_{G/K}(\exp(-X)\cdot o_K,h\cdot o_K ) \mid h\in H\}\leq C$であること」が満たされる．
