...
$G = \SU(1,2) $，$H= \SO(1,1)$の場合の証明を荒っぽく述べる．$X\in \pe\setminus\ha $に対して$Y(\real X) $が非有界であるとすると，任意の$\epsilon > 0$に対して，ある$t_{\epsilon}\in \real$が存在して「$ e^{Y(t_{\epsilon} X)}e^{Z(t_{\epsilon} X)}\cdot o_K $と$o_K$を結ぶ測地線」が「$e^{Y(t_{\epsilon} X)}\cdot o_K $と$o_K$を結ぶ測地線」が$o_K$でなす角が$\epsilon $未満となる．これは$X$と$\ha\setminus\{0\} $のなす角度の最小値が非零であることに矛盾し，\Cref{prob:1121}と同値な「$X\in \{0\}\cup \pe\setminus\ha $であることと$ Y(\real X) $が有界であることが同値である」ということが証明できる．
...
\begin{thm-a}(\cite[pp.~665--6]{ber88})
...
$G$を実簡約Lie群，$H$を$G$の閉部分群とし，ある可換部分群$B\subset G$が存在して$G = KBH $というCartan分解を持つとする．このとき

任意の$X\in \Lie B$に対して$Y(\real X) $が有界であれば，$G/H$がランク$d \defeq \dim B $となるための条件のうちの1つである，「ある定数$C > 0$が存在して，任意の$X\in \Lie B$に対し，$d_{G/K}(\exp(-X)\cdot o_K, o_K )  - \inf\{d_{G/K}(\exp(-X)\cdot o_K,h\cdot o_K ) \mid h\in H\}\leq C$であること」が満たされる．
