...
$G$を実簡約Lie群，$\Theta$を$G$のCartan対合とする．$H$を$G$の閉部分群とし，$\Theta$と可換な$G$の対合$\sigma$に対して$H = \sigma H$であるとする．極大分裂可換部分代数$\ah \subset \pe $のpositive Weyl chamber$\ah^{+} $を1つ固定すると，任意の$g\in  G $に対し，ある$k\in K$と$h\in H$および一意的な$b_g\in \exp\bar{\be^{+}} $が存在して$g = k_1b_gh$と書ける．この事実を用いてCartan射影$\nu\colon G\to \bar{\be^{+}} $を$\mu(g) = \log b_g $と定める．
