...
$G$を実簡約Lie群，$H$を$G$の閉部分群とし，$G$のCartan対合$\Theta$に対して$H = \Theta H$であるとする．極大分裂可換部分代数$\ah \subset \pe $のpositive Weyl chamber$\ah^{+} $を1つ固定すると，このとき任意の$g\in  G $に対し，に対するCartan射影$\nu\colon G\to \bar{\ah^{+}} $を
