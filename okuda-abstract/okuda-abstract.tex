...
\Cref{prob:1121}の背景を説明するために\cite{ber88}の内容のごく一部を述べる．\footnote{もう少し自然な言い回しを考えます．2022/01/11}\bluetext{$G$を実簡約Lie群，$H$を$G$の閉部分群とする．局所有界関数$r\colon G\to \real_{\geq 0} $がproperなradial functionであるとは，$r$が次を満たすことである．
  ...
  \item 任意の$R\geq 0$に対し，$B(R)\defeq  $
  \end{enumerate}
  radial function $r\colon G\to \real_{\geq 0} $から$r_{G/H}(gH)\defeq \inf_{h\in H}\{r(gh) \}$により定まる${r_{G/H}\colon G/H\to \real_{\geq 0}}$を$G/H$上のradial functionという．
  
  「$\real^d $と同じ増大度」を持つとき，$G/H$のランクが$d$であると言う．
