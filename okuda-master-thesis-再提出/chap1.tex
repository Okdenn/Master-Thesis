...
\begin{nttdef}\label{nttdef:fund}
  ...
  \item $G$を非コンパクト実線型簡約Lie 群とし，$H$を$G$の非コンパクトかつ連結成分有限個の閉部分群で，$G$のCartan対合$\Theta$に対して$\Theta H = H$なるものとする．
    \begin{itemize}
    \item 後述する\Cref{rem:loc-isom}より，$G$の線型性の仮定は外すことができる．
    \end{itemize}
...
\begin{npfwn}[\Cref{cor:prob-eg}]% \bluetext{make it precise}
  ...
  したがって後述の$G = \SO(1,2) $，$H = \SO(1,1) $の場合に$\pe_{H,\bdd} = \{0\}\cup \pe\setminus\ha $を示した\Cref{prop:classical-rank-one}と\Cref{thm:yos38}により，$Y(\real X)$が非有界であることと\Cref{eq:X-2}において$p_{l+1} = 0$かつ$p_1 \neq 0$であること，つまり$X\in \ha\setminus\{0\} $であることが同値である．よって\Cref{cor:prob-eg}が示された．
