...
\begin{rem}
  ...
  なぜならば，任意の線型とは限らない実半単純Lie群$G$に対し，$G$と次の\Cref{thm:loc-isom-linear}の意味で局所同型な実線型半単純Lie群$G_L$が存在し (\Cref{thm:yos38})，次の\Cref{lem:loc-isom}より$G$が線型の場合に帰着されるためである．
