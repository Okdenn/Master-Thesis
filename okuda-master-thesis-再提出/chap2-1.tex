...
\begin{rem}
  \Cref{thm:1216-main}では$G$の線型性を仮定したが，この仮定は不要である．つまり，$G$を実半単純Lie群としても\Cref{thm:1216-main}が成り立つ．

  なぜならば，$G$の中心$Z$ (離散群) の部分群で割ったときの$G/Z$は$G$と局所同型であるから，次の\Cref{lem:loc-isom}より$G$が線型の場合に帰着される．
  ...
  \begin{lem}\label{lem:loc-isom}
    $G_1$，$G_2$を局所同型な連結実半単純Lie群とし，局所同型を与える同型写像を$\phi\colon \ge_1\to \ge_2$とする．$\phi\colon \ge_1\to \ge_2$に対応する$G_1$と$G_2$の準同型を$\Phi\colon G_1\to G_2$とする．
  ...
  \begin{npfwn}{\Cref{lem:loc-isom}}
    ...
    $\phi|_{\pe_1} \colon \pe_1\to \pe_2$が内積を保つことと$\phi(\ha_1) = \ha_2 $より，$\phi(Y(X))\in \ha_2\cap \pe_2 $，$\phi(Z(X))\in \per{\ha_2}\cap \pe_2 $であり，したがって\Cref{thm:kob89}より$e^{\phi(X)}\cdot o_{K_2} =  $
  \end{npfwn}
\end{rem}
