...
\begin{rem}
  ...
  \begin{lem}\label{lem:loc-isom}
    $G_1$，$G_2$を局所同型な連結実半単純Lie群とする．そのCartan対合を$\Theta_1$，$\Theta_2$とする．閉部分群$H_1\subset G_1$，$H_2\subset G_2$同型写像$\phi\colon \ge_1\to \ge_2$に対し，$X\in \pe_1$に対し，$Y_1(\real X) $が有界であることと，$Y_2(\real \phi(X)) $が有界であることは同値である．
  \end{lem}
  \begin{npfwn}{\Cref{lem:loc-isom}}
    ...
    $\phi|_{\pe_1} \colon \pe_1\to \pe_2$が内積を保つことより，
  \end{npfwn}
\end{rem}
