...
\begin{rem}
  ...
  \begin{defi}
    実半単純Lie群$G_1$，$G_2$が局所同型であることを，それぞれのLie環$\ge_1$，$\ge_2$の間の同型写像$\phi\colon \ge_1\to \ge_2$とそのCartan分解$\ge_i = \ka_i\oplus \pe_i $とKilling形式に対し，$\pe_1$と$\pe_2$が等長同型であることを言う．
  \end{defi}

  \begin{lem}\label{lem:loc-isom}
    局所同型な実半単純Lie群$G_1$，$G_2$とその同型写像$\phi\colon \ge_1\to \ge_2$に対し，$X\in \pe_1$に対し，$Y_1(\real X) $が有界であることと，$Y_2(\real ) $
  \end{lem}
\end{rem}
