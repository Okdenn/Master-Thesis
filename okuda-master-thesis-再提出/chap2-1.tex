...
\begin{rem}\label{rem:loc-isom}
  ...
  \begin{thm}\label{thm:loc-isom-linear}
    任意の実半単純Lie環$\ge$に対し，ある$n\in \nat$と閉部分群$G_L\subset \GL(n,\real) $で$\ge\simeq \Lie G_L$なるものが存在する．
  \end{thm}
  \begin{npfwn}[\Cref{thm:loc-isom-linear}]
    Ado--岩澤の定理 (\cite[Theorem 5.61]{kando}) より，$\ge_{L}\simeq \ge$なる実半単純部分Lie環$\ge_{L} \subset \glie(n,\real) $が存在する．このとき，$\GL(n,\real) $での$\ge_{L} $の解析的部分群$G_{L}$は\Cref{thm:yos38}により閉部分群である．したがって\Cref{thm:loc-isom-linear}が示された．
  \end{npfwn}
  
  \begin{defi}\label{def:loc-isom}
    実半単純Lie群$G_1$，$G_2$が局所同型であることを，それぞれのLie環$\ge_1$，$\ge_2$の間の同型写像$\phi\colon \ge_1\to \ge_2$が存在し，Cartan分解$\ge_i = \ka_i\oplus \pe_i $，$i = 1,2$に対し，$\phi(\ka_1) = \ka_2 $，$\phi(\pe_1) = \pe_2 $かつ$\phi|_{\pe_1} \colon \pe_1\to \pe_2$がKilling形式の制限を保つことを言う．
  \end{defi}
