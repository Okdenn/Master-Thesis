...
\begin{rem}\label{rem:loc-isom}
  ...
  \end{defi}
  \begin{rem}
    実半単純Lie群$G_1$，$G_2$のそれぞれのLie環$\ge_1$，$\ge_2$がLie環として同型ならば，必要ならCartan対合を取り直し，Killing形式を定数倍することで\Cref{def:loc-isom}の意味でも$G_1$，$G_2$は局所同型である．この2つの操作は$Y(\real X) $の有界性の判定には影響しない．
  \end{rem}
...
\begin{thm}(\cite[p.~241, Theorem~3.1]{hel01})\label{thm:non-positivity}
  $G$を連結実半単純Lie群，$\ge = \ka\oplus \pe $を$\ge = \Lie G$のCartan分解とする．$K$を$\ka$をLie 環に持つ$G$の部分Lie 群とするとき，$G/K$は$\ge$のKilling形式から誘導されるRiemann多様体の構造を持ち，その断面曲率は至るところ非正である．
\end{thm}
