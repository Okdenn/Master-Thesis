...
\begin{rem}
  ...
  なぜならば，任意の線型とは限らない実半単純Lie群$G$に対し，$G$と次の\Cref{def:loc-isom}の意味で局所同型な実線型半単純Lie群$G_L$が存在し，次の\Cref{lem:loc-isom}より$G$が線型の場合に帰着されるためである．
  % の中心$Z$ (離散群) の部分群$Z_0$で割ったときの$G/Z_0$は$G$と局所同型であるから，

  \begin{thm}(\cite[pp. 219--20]{kando})
    任意の実半単純Lie環$\ge$に対し，線型Lie群$G_L$で，
  \end{thm}
