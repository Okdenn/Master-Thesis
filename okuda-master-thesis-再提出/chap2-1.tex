...
\begin{rem}\label{rem:loc-isom}
  \Cref{thm:1216-main}では$G$の線型性を仮定したが，この仮定は不要である．つまり，$G$を線型とは限らない実階数1の実半単純Lie群としても\Cref{thm:1216-main}が成り立つ．

  なぜならば，任意の線型とは限らない実半単純Lie群$G$に対し，$G$と次の\Cref{def:loc-isom}の意味で局所同型な実線型半単純Lie群$G_L$が存在し (\Cref{thm:loc-isom-linear})，次の\Cref{lem:loc-isom}より$G$が線型の場合に帰着されるためである．
  % の中心$Z$ (離散群) の部分群$Z_0$で割ったときの$G/Z_0$は$G$と局所同型であるから，

  \begin{lem}\label{thm:loc-isom-linear}
    任意の実半単純Lie環$\ge$に対し，ある$n\in \nat$と閉部分群$G_L\subset \GL(n,\real) $で$\ge\simeq \Lie G_L$なるものが存在する．
  \end{lem}
  \begin{npfwn}[\Cref{thm:loc-isom-linear}]
    Ado--岩澤の定理 (\cite[Theorem 5.61]{kando}) より，$\ge_{L}\simeq \ge$なる実半単純部分Lie環$\ge_{L} \subset \glie(n,\real) $が存在する．このとき，$\GL(n,\real) $における$\ge_{L} $の解析的部分群$G_{L}$は\Cref{thm:yos38}により閉部分群である．したがって\Cref{thm:loc-isom-linear}が示された．
  \end{npfwn}
  
  \begin{defi}\label{def:loc-isom}
    実半単純Lie群$G_1$，$G_2$が局所同型であることを，それぞれのLie環$\ge_1$，$\ge_2$の間の同型写像$\phi\colon \ge_1\to \ge_2$が存在し，Cartan分解$\ge_i = \ka_i\oplus \pe_i $，$i = 1,2$に対し，$\phi(\ka_1) = \ka_2 $，$\phi(\pe_1) = \pe_2 $かつ$\phi|_{\pe_1} \colon \pe_1\to \pe_2$がKilling形式の$\pe_1$，$\pe_2$への制限を保つことを言う．    
  \end{defi}
  \begin{rem}
    実半単純Lie群$G_1$，$G_2$のそれぞれのLie環$\ge_1$，$\ge_2$がLie環として同型ならば，必要に応じてCartan対合を取り直し，Killing形式を定数倍することで\Cref{def:loc-isom}の意味でも$G_1$，$G_2$は局所同型である．この2つの操作は$Y(\real X) $の有界性の判定には影響しない．
  \end{rem}
  ...
  \begin{lem}\label{lem:loc-isom}
    ...
    また$G_1$，$G_2$のCartan対合を$\Theta_1$，$\Theta_2$とする．閉部分群$H_1\subset G_1$，$H_2\subset G_2$は$\Theta_1H_1 = H_1$，$\Theta_2H_2 = H_2$，$\Phi(H_1) = H_2 $を満たすとする．

    このとき任意の$X\in \pe_1$に対し，$Y_1(\real X) $が有界であることと，$Y_2(\real \phi(X)) $が有界であることは同値である．ただし$Y_i$，$Z_i$，$i=1,2$は微分同相写像
    \begin{align*}
      \pi_i\colon (\ha_i\cap\pe_i)\oplus (\ha_i\cap\pe_i) &\to G_i/K_i ,\\
      (Y_i,Z_i) &\mapsto e^{Y_i}e^{Z_i}\cdot o_{K_i}
    \end{align*}
    により$(Y_i(X_i), Z_i(X_i)) \defeq \inv{\pi_i}(e^{X_i}\cdot o_{K_i}) $，$X_i\in \pe_i$と定める．
  \end{lem}
  \begin{npfwn}[\Cref{lem:loc-isom}]
    $\phi(\ka_1) = \ka_2 $より$\Phi(K_1) = K_2 $であるから，$\bar{\Phi}\colon G_1/K_1\to G_2/K_2 $，$\bar{\Phi} (g_1K_1) = \Phi(g_2)K_2  $，$g_1\in G_1$はwell-definedである．
    ...
    $\phi|_{\pe_1} \colon \pe_1\to \pe_2$が内積を保つことと$\phi(\ha_1) = \ha_2 $より，$\phi(Y_1(X))\in \ha_2\cap \pe_2 $，$\phi(Z_1(X))\in \per{\ha_2}\cap \pe_2 $であり，したがって\Cref{thm:kob89-lem6.1}より$Y_2(\phi(X)) = \phi(Y_1(X)) $，$Z_2(\phi(X)) = \phi(Z_1(X)) $である．

    $\phi|_{\pe_1} \colon \pe_1\to \pe_2$はKilling形式の$\pe_1$，$\pe_2$への制限を保つ写像であったから，$X\in \pe_1$に対して$Y_1(\real X) $が有界であることと，$Y_2(\real \phi(X)) $が有界であることは同値であり，\Cref{lem:loc-isom}が示された．
  \end{npfwn}

  $\phi(\pe_1) = \pe_2 $，$\phi(\ha_1) = \ha_2 $より$X\in \{0\}\cup\pe_1\setminus\ha_1 $と$\phi(X)\in \{0\}\cup \pe_{2}\setminus\ha_2 $は同値であるから，以上の議論をまとめて\Cref{rem:loc-isom}冒頭の主張が示された．
\end{rem}
...
\begin{thm}(\cite[p.~241, Theorem~3.1]{hel01})\label{thm:non-positivity}
  $G$を非コンパクトな連結実半単純Lie群，$\ge = \ka\oplus \pe $を$\ge = \Lie G$のCartan分解とする．$K$を$\ka$をLie 環に持つ$G$の部分Lie 群とするとき，$G/K$は$\ge$のKilling形式から誘導されるRiemann多様体の構造を持ち，その断面曲率は至るところ非正である．
\end{thm}
...
\begin{thm}({\cite[p.~245, Proposition~6.1]{hel01}})\label{thm:rank-of-symm-sp}
  $G/K$を\Cref{thm:non-positivity}の設定の通りとする．$G$の実階数と$G/K$内の全測地的で平坦な部分多様体の最大次元は一致する．
\end{thm}

\Cref{thm:rank-of-symm-sp}と\Cref{thm:visibility-and-rank}より$G$の実階数が1のとき$G/K$はvisibility manifoldである．よって\Cref{thm:1216-main}の設定のもとでは，角度を用いた$G = \SU(1,2) $，$H= \SO(1,1)$の場合の$\pe_{H,\bdd} = \{0\}\cup\pe\setminus\ha $の証明である\Cref{rem:su11-by-angle}と全く同様に，$X\in \pe\setminus\ha $の場合に$Y(\real X) $が非有界であると仮定して矛盾を示す論法で$\pe_{H,\bdd} =\{0\}\cup\pe\setminus\ha $が示される．
