...
\begin{rem}
  ...
  \begin{lem}\label{lem:loc-isom}
    $G_1$，$G_2$を\Cref{def:loc-isom}の意味で局所同型な実半単純Lie群とし，局所同型を与える同型写像を$\phi\colon \ge_1\to \ge_2$とする．$\phi\colon \ge_1\to \ge_2$に対応する$G_1$と$G_2$の準同型を$\Phi\colon G_1\to G_2$とする．

    また$G_1$，$G_2$のCartan対合を$\Theta_1$，$\Theta_2$とする．閉部分群$H_1\subset G_1$，$H_2\subset G_2$は$\Theta_1H_1 = H_1$，$\Theta_2H_2 = H_2$を満たし，$\Phi(H_1) = H_2 $を満たすとする．

    このとき$X\in \pe_1$に対し，$Y_1(\real X) $が有界であることと，$Y_2(\real \phi(X)) $が有界であることは同値である．
  \end{lem}
  \begin{npfwn}[\Cref{lem:loc-isom}]
    $G_1$，$G_2$の連結成分を$\phi(\ka_1) = \ka_2 $より$\Phi(K_1) = K_2 $であるから，$\bar{\Phi}\colon G_1/K_1\to G_2/K_2 $がwell-definedに定まる．
