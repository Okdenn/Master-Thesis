...
\begin{skpfwn}{\Cref{cor:prob-eg}}% \bluetext{make it precise}
  $\SO(1,n) $の$\real^{n+1} $への自然表現を用いて$\SO(1,n)/\SO(n)\simeq \{\trans{(x_1\cldots x_n)} \in \real^n \mid \sum_{1\leq i\leq n}x^2_i  < 1 \} $として実現する (後述する\Cref{nttdef:1127-main}と同様)．この実現において$\SO(1,k)/\SO(n) \simeq \{\trans{(x_1\cldots x_n)}\in \real^n \mid x_{k+1} = \cdots = x_n \text{ かつ } \sum_{1\leq i\leq n}x^2_i  < 1 \} $である．

  $e^X\cdot o_K $と$o_K$を結ぶ直線と$H\cdot o_K$で張られる超平面で$\SO(1,n)/\SO(n)$を切った際の断面を考える．
  \begin{figure}[H]
    \centering
    % \raggedleft
    % \raggedrightp
