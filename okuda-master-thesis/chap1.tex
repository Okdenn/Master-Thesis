...
\begin{rem}\label{rem:su11-by-angle}
  ...
  任意の$s\in \real, 0\neq r\in \real $に対し，
  \begin{align*}
    \lim_{s\to -\infty}\tan \phi_{s,\abs{r}} = \dfrac{-1}{\sinh 2\abs{r}}  \leq \tan \phi_{s,r} \leq  \lim_{s\to \infty}\tan \phi_{s,\abs{r}} = \dfrac{1}{\sinh 2\abs{r}}
  \end{align*}
  であるから，$X\nin \real Y $の元に対して$Y(\real X) $が非有界であるとすると，$\phi(X,\ha) >  \epsilon > 0$なる$\epsilon$に対し，ある$t\in \real $が存在して，$Y(tX) = sY $，$Z(tX) = rZ $に対し$\abs{\tan \phi_{s,r}} < \tan \epsilon $となる．しかし$\measuredangle_{o_K}(e^{tX}\cdot o_K) $$\phi(X,\ha)$未満となって矛盾する．
