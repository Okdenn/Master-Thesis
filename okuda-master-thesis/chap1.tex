...
\begin{lem}\label{lem:basic-prob}
  ...
  \item $G$が実階数1ならば，\Cref{cond:0117}と「$\pe_{H,\bdd} =  \{0\}\cup \pe\setminus\ha $」は同値である．
  \end{enumerate}
\end{lem}

いくつか補題を用意してから\Cref{lem:basic-prob}を示す．
...
\begin{npfwn}[\Cref{lem:0117-closed}]
  ...
  $\ge_{s}'$と$\ha $は共に$\theta$不変より$\theta \ha_s = \ha_s $であるから$H_s$の非コンパクト性を除いて\Cref{nttdef:fund}の条件すべてが成り立つ．
\end{npfwn}
...
\begin{defi}
  ...
  $H_s $が非コンパクトならば\Cref{lem:0117-closed}により，$(G_s, H_s, K_s\defeq G_s\cap K ) $に対して\Cref{thm:kob89-lem6.1}を用いて，$X\in \pe\cap\ge_{s}' $に対し
  \begin{align*}
...
\begin{npfwn}[\Cref{lem:0117-decomp}]
  ...
  $[X_c, X_s] = 0 $より$\exp(X)\cdot o_K = \exp(X_c)\exp(X_s)\cdot o_K $であり，$X,\ X'\in \ge'\cap\pe $に対し$\exp(X_c)\exp(X_s)\cdot o_K = \exp(X_c')\exp(X_s')\cdot o_K $ならば$\exp(X_c-X_c'+X_s)\cdot o_K = \exp(X_s')\cdot o_K $であり，$\ze(\ge')\cap \ge_s' = \{0\} $より$X_c = X_c' $，$\exp(X_s)\cdot o_K = \exp(X_s')\cdot o_K$である．
  ...
  \end{align*}
  であるから，$X_c = Y(X)_c + Z(X)_c $，$e^{X_s}\cdot o_K = e^{Y(X)_s}e^{Z(X)_s}\cdot o_K $である．
