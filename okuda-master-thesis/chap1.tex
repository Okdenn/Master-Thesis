...
\begin{defi}
  ...
  $G_s\supset H_s$，$X\in \pe\cap\ge_{s}' $に対し，$Y_s(X)\in \ha_s\cap\pe \subset \ha\cap \pe $と$Z_s(X)\in \per{\ha_s}\cap \pe \subset \per{\ha}\cap \pe $を次のように定める．
...
\begin{npfwn}[\Cref{lem:0117-decomp}]
  ...
  $[X_c, X_s] = 0 $より$\exp(X)\cdot o_K = \exp(X_c)\exp(X_s)\cdot o_K $であり，$\ze(\ge')\cap \ge_s' = \emptyset $より$\exp(X_c)\exp(X_s)\cdot o_K = \exp(X_c')\exp(X_s')\cdot o_K $ならば$X_c = X_c' $，$X_s = X_s'$である．
  ...
  \begin{align*}
    e^{X_c}e^{X_s}\cdot o_K &= e^{X}\cdot o_K \\
                            ...
                            &= e^{Y(X)_s + Z(X)_s}e^{Y(X)_c + Z(X)_c}\cdot o_K
  \end{align*}
  であるから，$X_c = Y(X)_s + Z(X)_s $，
