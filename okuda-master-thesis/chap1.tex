...
\begin{npfwn}[\Cref{lem:basic-prob}]
  ...
  \item 背理法による．$X\nin\per{\ha}\cap \pe $なる$X$に対し，$[X_1,X_2] = 0$ならば$e^{tX_1}e^{tX_2}\cdot o_K = e^{t(X_1 + X_2)}\cdot o_K = e^{tX}\cdot o_K$である．したがって\Cref{thm:kob89-lem6.1}より$Y(tX) = tX_1 $，$Z(tX) = tX_2 $であることから$Y(\real X) = \real X_1 $となり，$X_1\neq 0$より$Y(\real X)$は有界集合とならない．
    ...
    また，$X\nin\per{\ha}\cap \pe $なる$X$に対し，$\ze_{\ze(\ha)\cap\pe}(X) \neq 0$ならば，ある$0\neq W\in \ze(\ha)\cap\pe $が存在して，$[W, X] = [W, X_2] = 0 $である．$X = W+X_2 $とすると，
  ...
  \item 次を示せばよい．
    ...
    \end{align}
    $G$の実階数が1かつ$H$が非コンパクトであるから$\ha\cap\pe$は極大分裂可換部分代数であり，右辺の2つ目の集合は$\ze(\ha)\cap\pe \subset \real X_{1} $より$\{X\in \pe\mid  [X_1, X_2]\neq 0 \text{ である．} \}$となる．よって\Cref{eq:basic-prob}の右辺$\{X\in \pe\mid X_1 = 0\text{ または }  [X_1, X_2]\neq 0 \text{ である．} \}$は$X\in \{0\}\cup\pe\setminus\{0\} $と同値であり，\Cref{eq:basic-prob}が示された．
  \end{enumerate}  
\end{npfwn}
