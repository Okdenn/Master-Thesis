...
\begin{npfwn}[\Cref{lem:basic-prob}]
  ...
  \item $G$が実階数1のときに$X\in \pe$に対して$X\in \{0\}\cup \pe\setminus \ha $であることと\Cref{cond:0117}を満たすことが同値であることを示せば良い．$G$の実階数が1かつ$H$が非コンパクトで$\ha = \theta\ha$であるから$\ha\cap\pe$は極大分裂可換部分代数である．
    ...
    \Cref{cond:0117}を満たさない$X\in \pe$を任意に1つ固定する．\Cref{cond:0117}の \cnum{1} が成り立たないことから$X\nin \pe\setminus\per{\ha}$である．また$X$は\Cref{cond:0117}の \cnum{2}，\cnum{3} を満たさない．もし$[X_1, X_2] = 0 $ ならば$\ha\cap\pe$が極大分裂可換部分代数であることから$X\in\ha\cap \pe $であるから，$X\in \ha\setminus\{0\} $が言える．
