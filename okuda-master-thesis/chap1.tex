...
\begin{npfwn}[\Cref{lem:0117-decomp}]
  ...
  前者より$e^{X_c} \cdot o_K = e^{Y(X)_c}e^{Z(X)_c}\cdot o_K $と$Y(X)_c \in \ha\cap \pe$，$  Z(X)_c \in \per{\ha}\cap \pe$であるから$Y(X_c) = Y(X)_c $を得る．また後者と$Y_s(X) $の定義より$Y_s(X_s) = Y(X)_s $を得る．

  よって$Y(X) = Y(X)_c + Y(X)_s = Y(X_c) + Y(X_s) $であり，\Cref{lem:0117-perp}の仮定より$X_c\in \ze(\ge')\cap \pe\subset \ha \cap \pe $であるから$Y(X_c) = X_c $を得，\Cref{lem:0117-decomp}の主張を得る．  
\end{npfwn}
