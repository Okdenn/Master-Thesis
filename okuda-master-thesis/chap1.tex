...
\begin{npfwn}[\Cref{lem:basic-prob}]
  ...
  \item 次を示せばよい．
    ...
    \end{align}
    $G$の実階数が1かつ$H$が非コンパクトであるから$\ah\subset \ha\cap\pe$を極大分裂可換部分代数としたとき，$\ze(\ha)\cap\pe \subset \ah $，$\ha\cap \pe\subset \ah $である．よって$\ze_{\ze(\ha)\cap\pe}(X)  = \{0\} $と$X\nin \ha$は同値であるから，\Cref{eq:basic-prob}の右辺は$X\in \ha\setminus\{0\} $と${\{X\in \pe\mid X_1 \neq 0 \text{ あるいは } [X_1, X_2] = 0 \}} $は
  \end{enumerate}  
\end{npfwn}
