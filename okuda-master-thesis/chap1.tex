...
\begin{defi}
  ...
  $H_s $が非コンパクトならば\Cref{lem:0117-closed}により，$(G_s, H_s) $に対して\Cref{thm:kob89-lem6.1}を用いて，$X\in \pe\cap\ge_{s}' $に対し
  \begin{align*}
...
\begin{npfwn}[\Cref{lem:0117-decomp}]
  ...
  $[X_c, X_s] = 0 $より$\exp(X)\cdot o_K = \exp(X_c)\exp(X_s)\cdot o_K $であり，$X,\ X'\in \ge'\cap\pe $に対し$\exp(X_c)\exp(X_s)\cdot o_K = \exp(X_c')\exp(X_s')\cdot o_K $ならば$\exp(X_c-X_c'+X_s)\cdot o_K = \exp(X_s')\cdot o_K $であり，$\ze(\ge')\cap \ge_s' = \{0\} $より$X_c = X_c' $，$X_s = X_s'$である．
...
\begin{npfwn}[\Cref{lem:basic-prob}]
  ...
  \item $G$が実階数1のときに$X\in \{0\}\cup \pe\setminus \ha $であることと\Cref{cond:0117}を満たすことが同値であることを示せば良い．

    $X\in\ha\setminus\{0\} $ならば\Cref{cond:0117}を満たさ\textbf{ない}ことは明らかであるから，\Cref{cond:0117}を満たさ\textbf{ない}ならば$X\in\ha\setminus\{0\} $であることを示せば良い．
