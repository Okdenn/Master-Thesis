...
\begin{npfwn}[\Cref{lem:basic-prob}]
  ...
  \item 背理法による．$X\nin\per{\ha}\cap \pe $なる$X$に対し，$[X_1,X_2] = 0$ならば$e^{tX_1}e^{tX_2}\cdot o_K = e^{t(X_1 + X_2)}\cdot o_K = e^{tX}\cdot o_K$である．したがって\Cref{thm:kob89-lem6.1}より$Y(tX) = tX_1 $，$Z(tX) = tX_2 $であることから$Y(\real X) = \real X_1 $となり，$X_1\neq 0$より$Y(\real X)$は有界集合とならない．
    ...
    また，$X\nin\per{\ha}\cap \pe $なる$X$に対し，$\ze_{\ze(\ha)}(X) \neq 0$ならば，ある$0\neq W\in \ze(\ha) $が存在して，$[W, X] = [W, X_2] = 0 $である．$X = W+X_2 $とすると，
  ...
  \item 次を示せばよい．
    ...
    \end{align}
    % $G$の実階数が1であるから$\ze(\ha) = 0 $であることと$\dim \ha\cap\pe \geq 2$であることは同値である．
    ...
    $\ze(\ha) \neq \{0\} $のとき，$\ha\cap\pe\neq \{0\} $と$G$の実階数が1であることにより，$\ha\subset \pe $かつ$\dim \ha = 1$である．このとき$X\in \pe\setminus\ha$を任意に1つ固定する．$X\in\per{\ha}\cap\pe $ならば\Cref{eq:basic-prob}の右辺に含まれるから，$X\nin \per{\ha} $とする．このとき$X_1\neq 0 $，$X_2\neq 0 $であり，$G$の実階数が1であるから$[X_1,X_2]\neq 0 $である．
  \end{enumerate}  
\end{npfwn}
