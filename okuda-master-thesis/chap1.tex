...
\begin{thm}\label{thm:1018-main}
  ...
  $
  \begin{cases}
    \ds Y(X) &= \frac{1}{4}\inv{\tanh}[\tanh(4\norm{X})\cos\theta ]\frac{\pr(X)}{\norm{\pr(X)}},\quad X\in \pe\setminus (\per{\solie(k,1)}\cap \pe) \\
    \ds Y(X)&= 0,\qquad \qquad\qquad\qquad\qquad\qquad\qquad\quad \ X\in  \per{(\solie(k,1))}\cap \pe
  \end{cases}
  $である．
\end{thm}
\begin{cor}
  $X\in \pe\setminus \solie(k,1) $
  % $ e^{X}\cdot o_K\nin \SO(k,1)\cdot o_K $
  のとき，$0 \leq \cos \theta\neq 1$であり，\\
  $Y(\real X) = \lbig\{s\frac{\pr(X)}{\norm{\pr(X)}} \mid  -\frac{\inv{\tanh}(\cos\theta)}{4} < s < \frac{\inv{\tanh}(\cos\theta)}{4} \rbig\} $ (if $X\in \pe\setminus (\per{\solie(k,1)}\cap \pe)$) \\
  あるいは$Y(\real X) = \{\bm{0}\}$ (if $X\in  \per{(\solie(k,1))}\cap \pe$) より，$\solie(k,1)\cap \pe $内で有界である．
\end{cor}
...
\begin{defi}\textcolor{white}{hoge}
  ...
  \begin{easylist}[itemize]
    ...
    @ $\pr\colon \pe = (\solie(k,1)\cap \pe)\oplus (\per{(\solie(k,1) )}\cap \pe) \to (\solie(k,1)\cap \pe)$を直交射影とする．
