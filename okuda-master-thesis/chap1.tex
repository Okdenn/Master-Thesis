...
\begin{nttdef}
  ...
  \item $G$を非コンパクト実半単純Lie 群，$H$を$G$の非コンパクトな部分Lie群で，$G$のCartan対合$\Theta$に対して$\Theta H = H$なる実半単純Lie群とする．
...
\begin{prob}(by T.~Kobayashi)\label{prob:1121}  
  ...
  \begin{enumerate}
  \item \begin{align}
          \pe_{H,\bdd} = (\per{\ha}\cap\pe) \cup \{X\in \pe\mid  [X_1, X_2]\neq 0 \text{ かつ } \ze_{\ze(\ha)}(X)  = 0\text{ である．} \}\label{eq:prob-1121}
        \end{align}
        であるか？
...
\begin{lem}\label{lem:basic-prob}
  ...
  \item $G$が実階数1の実半単純Lie群ならば\Cref{prob:1121}と「$\pe_{H,\bdd} =  \{0\}\cup \pe\setminus\ha $」は同値である．
  \end{enumerate}
\end{lem}
...
\begin{npfwn}[\Cref{lem:basic-prob}]
  \leavevmode\vspace{-1em}
  \begin{enumerate}
  \item 背理法による．$X\nin\per{\ha}\cap \pe $なる$X$に対し，$[X_1,X_2] = 0$ならば$e^{tX_1}e^{tX_2}\cdot o_K = e^{t(X_1 + X_2)}\cdot o_K = e^{tX}\cdot o_K$である．したがって\Cref{thm:kob89-lem6.1}より$Y(tX) = tX_1 $，$Z(tX) = tX_2 $であることから$Y(\real X) = \real X_1 $となり，$X_1\neq 0$より$Y(\real X)$は有界集合とならない．

    また，$X\nin\per{\ha}\cap \pe $なる$X$に対し，$\ze_{\ze(\ha)}(X) \neq 0$ならば，ある$0\neq W\in \ze(\ha) $が存在して，$W = W_1 + W_2 $
  \item $X_1 = 0$であることと$ X\in \per{\ha}\cap\pe $であることは同値であることから従う．
  \item 次を示せばよい．
    \begin{align}
      \{0\}\cup \pe\setminus \ha = (\per{\ha}\cap\pe) \cup \{X\in \pe\mid  [X_1, X_2]\neq 0 \text{ かつ } \ze_{\ze(\ha)}(X)  = 0\text{ である．} \}\label{eq:basic-prob}
    \end{align}
    % $G$の実階数が1であるから$\ze(\ha) = 0 $であることと$\dim \ha\geq 2$であることは同値である．
    $\ze(\ha) = 0 $ならば\Cref{eq:basic-prob}の右辺は$\{X\in \pe\mid X_1 = 0\text{ あるいは } [X_1, X_2]\neq 0 \} $と一致する．$G$の実階数が1であることにより$X\in \ha\setminus\{0\} $であることと$X_1\neq 0 $かつ$[X_1, X_2] = 0 $であることは同値であるから，
    \begin{align*}
      \{0\}\cup \pe\setminus \ha = \{X\in \pe\mid X_1 = 0\text{ あるいは } [X_1, X_2]\neq 0 \} 
    \end{align*}
    であり，\Cref{eq:basic-prob}が示された．

    $\ze(\ha) =  $$X\in \pe\setminus\ha$を任意に1つ固定する．$X\in\per{\ha}\cap\pe $ならば\Cref{eq:basic-prob}の右辺に含まれるから，$X\nin \per{\ha} $とする．このとき$X_1\neq 0 $，$X_2\neq 0 $であり，$G$の実階数が1であるから$[X_1,X_2]\neq 0 $である．
  \end{enumerate}  
\end{npfwn}
...
\begin{npfwn}[\Cref{cor:prob-eg}]\bluetext{より正確に記述せよ}
...
\subsection{\Cref{prob:1121} の観察: \Cref{prob:1121}の仮定を外した場合の成り立たない例}

\Cref{prob:1121}と次の\Cref{prob:1121-2}は同値である．
...
つまり\Cref{prob:1101}の右辺を次の\Cref{prob:1101-2}のように少し弱めても\Cref{lem:1118-main}はその反例になっている．
