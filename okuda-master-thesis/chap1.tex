...
\begin{cor}
  $X\in \pe\setminus \ha $
  % $ e^{X}\cdot o_K\nin \SO(k,1)\cdot o_K $
  のとき，$0 \leq \cos \theta\neq 1$であり，$X\in \pe\setminus (\per{\ha}\cap \pe)$ならば
  \begin{align*}
    Y(\real X) = \lbig\{s\frac{\pr(X)}{\norm{\pr(X)}} \relmiddle|  -\frac{\inv{\tanh}(\cos\theta)}{4} < s < \frac{\inv{\tanh}(\cos\theta)}{4} \rbig\}
  \end{align*}  
