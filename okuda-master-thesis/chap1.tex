...
\begin{rem}\label{rem:su11-by-angle}
  ...
  \begin{lem}\label{lem:0106}
    ...
    \begin{pmatrix}
      \cosh t & \sinh t\\ \sinh t & \cosh t
    \end{pmatrix}
    \sqrt{-1}\tanh s \in \SU(1,1)/\U(1) $，$t > 0$，$s\in \real$に対し，$o_K $と$e^{tY}e^{sZ}\cdot o_K$を結ぶ測地線が$o_K$と$e^{tY}\cdot o_K $を結ぶ測地線となす角$\phi_{s,t} $は，$\tan \phi_{s,t} = \dfrac{\tanh 2s}{\sinh 2t} $を満たす．
  \end{lem}
  ただし，$Y \defeq
  \begin{pmatrix}
    0 & 1\\ 1 & 0
  ...
  \end{align*}
  であるから，$X\nin \real Y $の元に対して$Y(\real X) $が非有界であるとすると，$\ha$と$X$の角度未満のある数$\epsilon > 0$に対し，ある$r\in \real $が存在して，$Y(rX) = tY $，$Z(rX) = sZ $に対し$\abs{\tan \phi_{s,t}} < \tan \epsilon $となり，
...
\subsection{\Cref{yosou:1121} の観察: \Cref{yosou:1121}はなぜこの形になったか}
...
以上のことから\Cref{yosou:1121}が立てられた．
