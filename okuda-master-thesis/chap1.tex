...
\begin{nttdef}\label{nttdef:fund}
  ...
  \item $G$を非コンパクト実線型簡約Lie 群，$H$を$G$の非コンパクトかつ連結成分有限個の閉部分群で，$G$のCartan対合$\Theta$に対して$\Theta H = H$なるものとする．このような$H$を$G$の簡約な部分群と呼ぶ．
...
\begin{prob}(小林俊行氏による)\label{prob:1121}
  ...
  \begin{enumerate}
  \item $G$が実半単純Lie群ならば$\pe\setminus\pe_{H,\bdd} $はLebesgue速度に対して測度0であるか？
...
\begin{lem}\label{lem:basic-prob}
  ...
  \item \Cref{prob:1121}において，2.が成り立つならば1.が成り立つ．
...
\begin{npfwn}[\Cref{lem:0117-perp}]
  ...
  任意の$X\in \per{\ha_s}\cap \ge'_s\cap\pe $と$Z\in \per{\ha}\cap\pe $の元を取る．$Z$を\Cref{eq:0117-perp}の分解に対応して$Z = Z_s + Z_{\ka} + Z_{c} $と分解する．
  ...
  \end{align}
  である．ここで$Z_c\in \ze(\ge') $より$[T, Z_c ]= 0$であるから\Cref{eq:0117-perp-2}の左辺は0となり，\Cref{lem:0117-perp}の主張を得る．
\end{npfwn}
...
\begin{npfwn}[\Cref{lem:0117-closed}]
  ...
  $\ge_{s}'$と$\ha $は共に$\theta$不変より$\theta \ha_s = \ha_s $であるから\Cref{lem:0117-perp}，特に$H_s$の非コンパクト性を除いて\Cref{nttdef:fund}の条件すべてが成り立つ．
\end{npfwn}
...
\begin{npfwn}[\Cref{lem:0117-decomp}]
  ...
  前者より$e^{X_c} \cdot o_K = e^{Y(X)_c}e^{Z(X)_c}\cdot o_K $であり，\Cref{lem:0117-perp}の仮定より$Y(X)_c \in \ha\cap \pe$，$  Z(X)_c \in \per{\ha}\cap \pe$であるから$Y(X_c) = Y(X)_c $を得る．さらに\Cref{lem:0117-perp}の仮定より$X_c\in \ze(\ge')\cap \pe\subset \ha \cap \pe $であるから$Y(X_c) = X_c = Y(X)_c $を得る．

  また後者と$Y_s(X) $の定義より$Y_s(X_s) = Y(X)_s $を得る．よって$Y(X) = Y(X)_c + Y(X)_s = Y(X_c) + Y_s(X_s) = X_c + Y_s(X_s) $であり，\Cref{lem:0117-decomp}の主張を得る．  
\end{npfwn}
...
\begin{npfwn}[\Cref{lem:basic-prob}]
  ...
  \item $X\in \pe$が\Cref{cond:0117}を満たさないとき，$X\in \pe\setminus\pe_{H,\bdd} $であることを示せば良い．
    ...
    以上より主張が従う．
  \item $X_1 = 0$であることと$ X\in \per{\ha}\cap\pe $であることが同値であり$X\in \per{\ha}\cap\pe $ならば\Cref{thm:kob89-lem6.1}より$Y(X) = 0 $であることから従う．
  \item $G$が実階数1のときに$X\in \pe$に対して$X\in \{0\}\cup \pe\setminus \ha $であることと\Cref{cond:0117}を満たすことが同値であることを示せば良い．

    $X\in(\ha\cap\pe)\setminus\{0\} $ならば\Cref{cond:0117}を満たさ\textbf{ない}ことは明らかであるから，\Cref{cond:0117}を満たさ\textbf{ない}ならば$X\in(\ha\cap\pe)\setminus\{0\} $であることを示せば良い．

    
    
    \Cref{cond:0117}を満たさない$X\in \pe$を任意に1つ固定する．\Cref{cond:0117}の \cnum{1} が成り立たないことから$X\nin \pe\setminus\per{\ha}$である．また$X$は\Cref{cond:0117}の \cnum{2}，\cnum{3} を満たさない．もし$[X_1, X_2] = 0 $ ならば$X\in\ha\cap \pe $であるから，$X\in \ha\setminus\{0\} $が言える．

    以上より$[X_1,X_2] \neq 0$かつ$X\nin \pe\setminus\per{\ha}$なる$X$のみを考えれば十分である．このとき$X$は\Cref{cond:0117}の \cnum{2}，\cnum{3} を満たさないことからさらに$X\nin \ge_{s}' $かつ$\pe\cap\ze(\ge')\subset \ha $である．$X  = X_c + X_s $に対し，前者より$X_c\neq 0 $であり，後者より$X_c\in \pe\cap\ze(\ge')\subset \ze_{\ha\cap\pe}(X) $である．しかし，$G$の実階数が1かつ$H$が非コンパクトで$\ha = \theta\ha$であるから$\ha\cap\pe$は極大分裂可換部分代数であり，$X\nin\ha\setminus\{0\} $ならば$[X \ha\cap\pe ] \neq  \{0\}$より，$0 = X_c \in \ze_{\ha\cap\pe}(X) $とならざるを得ない．これは$X_c\neq 0 $に矛盾し，したがって$X \in \ha\setminus\{0\}  $である．
...
\begin{npfwn}[\Cref{lem:riem-metric-su11}]
  ...
  \end{align*}
  なるような計量$g $がKilling形式から誘導される計量であるが，それが主張の形であることを示せば良い (\Cref{fig:riem-metric-su11}参照)．
...
\begin{npfwn}[\Cref{prop:prob-eg}]
  ...
  \Cref{fig:prob-eg-1}では，左の円が{\Poincare}円板$G/K$，右の円は$e^{tX_{\theta}}\cdot o_K $を通り$H\cdot o_K$に垂直に交わる測地線を延長した円を描いた．$G/K$の測地線は境界に直交する円弧であるから，この2つの円は直交している．
...
\begin{rem}\label{rem:su11-by-angle}
  ...
  \Cref{prop:prob-eg}は角度を用いた議論によっても示すことができる．具体的には，計算により次の\Cref{lem:0106}が示せる (計算は省略する)．
...
\begin{cor}\label{cor:prob-eg}
  $G = \SO(1,n) $，$H = \SO(1,k) $，$1\leq k\leq n-1$は\Cref{prob:1121}の肯定的な例である．具体的には$\pe_{H,\bdd} = \{0\}\cup \pe\setminus\ha $である．% より具体的には次の\Cref{thm:1018-main}が成り立つ．
\end{cor}
...
\begin{skpfwn}{\Cref{cor:prob-eg}}% \bluetext{make it precise}
  $\SO(1,n) $の$\real^{n+1} $への自然表現を用いて$\SO(1,n)/\SO(n)\simeq \{\trans{(x_1\cldots x_n)} \mid \sum_{1\leq i\leq n}x^2_i  < 1 \} $として実現する．$e^X\cdot o_K $と$o_K$を結ぶ直線と$H\cdot o_K$で張られる超平面で$\SO(1,n)/\SO(n)$を切った際の断面を考える．
  \begin{figure}[H]
    \centering
    % \raggedleft
    % \raggedrightp
...
\subsection{\Cref{prob:1121} の観察: 類似の問とその反例}
...
ここで$G$の実階数が2の場合には\Cref{eq:prob-1121-2}は成り立たないことがわかっている．
\input{chap3.tex}


$G$が実階数1の場合に限っても\Cref{prob:1121-2}と類似の問題はいくつか考えられる．例えば\Cref{eq:prob-1121-3}において$\ha\cap\pe$を$\ha$に置き換えた次の問題が立てられる．
