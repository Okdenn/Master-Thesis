...
\begin{pfwn}{\Cref{prop:1127-main}}
  ...
  \begin{pmatrix}
    ...
    \bar{z} & 0 & 0 \\
    \bar{w} & 0 & 0 
  \end{pmatrix} \in \per{\ha}\cap\pe $，$z,w\in \quat \st \abs{z}^2 +\abs{w}^2  = 1 $と$r\in \real$により$rZ$と表せる．$Z\in\pe\setminus \ha $より$\re z \neq \pm 1$であることに注意する ($\re\colon \quat\ni a+bi+cj+dk\mapsto a\in \real$とする)．$Z(t_n) = r_nZ_n$，$Z_n\defeq \begin{pmatrix}
    0 & z_n & w_n \\
    \bar{z_n} & 0 & 0 \\
    \bar{w_n} & 0 & 0 
  \end{pmatrix} $とする．\bluetext{部分列を取って$z_n,w_n$が収束するようにする．}
  ...
  \begin{align*}
    ...
    \end{pmatrix}
              \lbig[\begin{pmatrix}
                1\\ \pm \bar{z} \tanh \abs{r_n}  \\ \pm \bar{w} \tanh \abs{r_n}
              \end{pmatrix}\rbig]\\
    &=  \lbig[ \begin{pmatrix}
      \cosh s_n \pm \bar{z} \tanh \abs{r_n} \sinh s_n \\ \sinh s_n \pm \bar{z} \tanh \abs{r_n} \cosh s_n \\ \pm \bar{w} \tanh \abs{r_n}
    \end{pmatrix}\rbig],
  \end{align*}
  複号は$r_n$の符号$\pm$と同順，である．このとき$\lim_{n\to \infty}\tanh s_n = 1 = \lim_{n\to \infty}\tanh \abs{r_n} $と$\lim_{n\to \infty} \re z_n = \re z \neq \pm 1$に注意すると次を得る．具体的な計算は後述する．
  \begin{align}
    \lim_{n\to \infty}(\sinh s_n \pm \bar{z} \tanh \abs{r_n} \cosh s_n)\inv{(\cosh s_n \pm \bar{z} \tanh \abs{r_n} \sinh s_n) } = 1\label{eq:-1}
  \end{align}
  である．
  ...
  \in \quat\mathbb{H}^2 $から\\
  $ \begin{pmatrix}
    (\sinh s_n \pm \bar{z} \tanh \abs{r_n} \cosh s_n)\inv{(\cosh s_n \pm \bar{z} \tanh \abs{r_n} \sinh s_n) } \\  \ast 
  ...
  \in \quat\mathbb{H}^2 $から$e^{t_nX}\cdot o_K \in \quat \mathbb{H}^2 $へのベクトルがなすEuclideanな内積の値$J_n$は，$X \defeq   \begin{pmatrix}
    ...
    \bar{z_0} & 0 & 0 \\
    \bar{w_0} & 0 & 0 
  \end{pmatrix}$，$z_0,w_0\in \quat \st \abs{z_0}^2 +\abs{w_0}^2  = 1 $とするとき$J_n = \dfrac{\bar{z_0}}{r_0}\tanh (tr_0) $，$r_0\defeq \sqrt{\abs{z_0}^2 + \abs{w_0}^2 } $であり，$\lim_{n\to \infty}J_n = \dfrac{\bar{z_0}}{r_0}\neq 1 $であり，

  以上2つを合わせると$e^{s_n A}e^{r_n Z_n}\cdot o_K = e^{t_n X}\cdot o_K\implies \lim_{n\to \infty} I_n = \lim_{n\to \infty}J_n = 1 $となり矛盾する．
...
\begin{calcof}{\Cref{prop:1127-main}}
  ...
  $\lim_{n\to \infty}\abs{(\sinh s_n \pm \bar{z} \tanh \abs{r_n} \cosh s_n)\inv{(\cosh s_n \pm \bar{z} \tanh \abs{r_n} \sinh s_n) } - 1} = 0$を示せば主張が得られる．具体的に計算すると，
  \begin{align*}
    &\lim_{n\to \infty}\abs{(\sinh s_n \pm \bar{z} \tanh \abs{r_n} \cosh s_n)\inv{(\cosh s_n \pm \bar{z} \tanh \abs{r_n} \sinh s_n) } - 1}\\
    % &\qquad\qquad\qquad \text{($\text{(この極限)} = 0$を示せば良い)} \\
    = & \lim_{n\to \infty}\lbig|\frac{(\tanh s_n\pm \bar{z}\tanh \abs{r_n})(1 \pm z \tanh\abs{r_n}\tanh s_n )}{\abs{(1 \pm \bar{z} \tanh\abs{r_n}\tanh s_n )}^2} - 1 \rbig|\\    
    = & \lim_{n\to \infty}\frac{\abs{(\tanh s_n\pm \bar{z}\tanh \abs{r_n})z' - (1 \pm \bar{z} \tanh\abs{r_n}\tanh s_n )z' }}{\abs{(1 \pm \bar{z} \tanh\abs{r_n}\tanh s_n )}^2}, \quad z'\defeq 1 \pm z\tanh\abs{r_n}\tanh s_n \\
    = & \lim_{n\to \infty}\frac{\abs{(1-\tanh s_n)(-1\pm \bar{z}\tanh \abs{r_n} )z'}}{\abs{(1 \pm \bar{z} \tanh\abs{r_n}\tanh s_n )}^2} \\
    = & \lim_{n\to \infty}\frac{\abs{(1-\tanh s_n)(-1\pm \bar{z}\tanh \abs{r_n} )}}{\abs{(1 \pm \bar{z} \tanh\abs{r_n}\tanh s_n )}} 
  \end{align*}
  であり，
  $0 < \min{\abs{1\pm \re z}}\leq  \abs{(1 \pm \bar{z} \tanh\abs{r_n}\tanh s_n )}\leq \sqrt{2^2 + 1^2} = \sqrt{5} $と$\min\{\abs{-1\pm \re z } \}  \leq \abs{-1\pm\bar{z}\tanh \abs{r_n} } \leq \sqrt{5} $であることから，
    \begin{align*}
      0 = \lim_{n\to \infty}(1-\tanh s_n)\frac{\min\{\abs{-1\pm \re z} \}}{\sqrt{5}}  &\leq \lim_{n\to \infty}\frac{\abs{(1-\tanh s_n)(-1\pm \bar{z}\tanh \abs{r_n} )}}{\abs{(1 \pm \bar{z} \tanh\abs{r_n}\tanh s_n )}}\\
