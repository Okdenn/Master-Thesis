...
\begin{nttdef}\label{nttdef:su21-red}
  ...
  \item $A_{\alpha}\in \ah $を，任意の$H\in \ah$に対して$B(H,A_{\alpha}) = \alpha(H) $を満たす元とする．
    ...
    このとき，任意の$H\in \ah$に対して$B(H, [X_{\alpha}, \theta X_{\alpha}]) = \alpha(H) B(X_{\alpha}, \theta X_{\alpha}) $，したがって
    \begin{itemize}
    \item $[X_{\alpha}, \theta X_{\alpha}] = B(X_{\alpha}, \theta X_{\alpha})A_{\alpha} $，
    \item $[Y_{\alpha}, \theta Y_{\alpha}]  = B(Y_{\alpha}, \theta Y_{\alpha})A_{\alpha} $，$ [X_{2\alpha}, \theta X_{2\alpha}] = 2B(X_{2\alpha}, \theta X_{2\alpha})A_{\alpha} $
    \end{itemize}
    である．
...
\begin{npfwn}[\Cref{lem:su11}]
  ...
  \begin{align*}
    ...
    c_{\alpha} X_{-\alpha} &\leftrightarrow
      ...
      \end{pmatrix} 
  \end{align*}
  により与えると，\bluetext{これは$\ge'$と$\sulie(1,1)$の間の同型になっている．チェックせよ}
\end{npfwn}
