...
\begin{pfwn}{\Cref{thm:1216-main}}
  ...
  \begin{case}
    ...
    \Cref{cor:sub-lie-alg}により$X$と$\ha $を含む部分Lie環$\ge' \subset \ge $で$\sulie(1,1) $に同型なものが存在する．$\ge' $に対応する$G$の解析的部分群を$G'$とし，その岩澤分解を$G' = K'A'N' $とする．
    
    $A_{\alpha}\in \ha $を任意の$H\in \ha \defeq  $に対して$B(H,A_{\alpha}) = \alpha(H) $を満たす元とすると， 任意の固定した$\tau \in \real$に対し，$Y(\tau X) $に対応する$\ge_{\tau} $の元は$\dfrac{B(Y(\tau X), A_{\alpha}) }{\norm{A_{\alpha}}}
    \begin{pmatrix}
      0 & 1 & 0\\
      1 & 0 & 0\\
      0 & 0 & 0\\
